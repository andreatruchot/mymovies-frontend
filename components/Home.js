...
function Home() {
  ...
  const [moviesData, setMoviesData] = useState([]); // Initialise le state pour stocker les données des films
  ...
  const updateLikedMovies = (movieTitle) => {
   ...
   useEffect(() => {
    fetch('https://mymovies-backend-ten.vercel.app/movies') 
      .then(response => response.json())
      .then(data => {
        setMoviesData(data.movies);
      })
      .catch(error => console.error('Erreur lors de la récupération des données:', error)); 
  }, []); // Le tableau vide assure que l'effet ne s'exécute qu'au montage du composant
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie 
    key={i} 
    updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview}  poster_path={data.poster_path} vote_average={data.vote_average} vote_count={data.vote_count} />;
  });
